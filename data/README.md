...
- *LatencyMarkers*:
	...
	- The `condition_advance..` marker indicates the latency of the previous condition, while the `latency_applied` marker is sent when the new latency is applied before starting the next condition.
- *RokokoViveRaw*: 
...
The later files will contain 2 extra streams (these do not need to be analysed for the IN5060 assignment):
- *FrameMarker1*:
	- contains frame numbers for a video file
- *LiveAmp LiveAmp32 102104-1074*:
	- contains EEG data of 32 channels
...
| 003		  | The recording was started after the practice round, so the first `latency_applied` marker was not included in this recording (the `repetition_start` is also missing). But the first condition was the 200 ms added latency indicated by the `condition_advance..` marker and that the other condition options are followed after. So the data that comes before the first `condition_advance` and next `latency_applied` belonged to the first condition with 200 ms for this participant.| 
...
| 020		  | Recording was started after the 1st trial.|
| 022		  | The recording was stopped after 5 tries at the end, but the 5th condition of the JTHFT took 117 seconds, this was recorded manually. |
| 031         | In the 3rd trial of JTHFT, the end time should be the acceptance time instead of the end time.  |
| 032		  | The experiment was aborted and not completed. |
| 035		  | Wrist in static position with the hand facing down.  |

## Contact

If you have any questions about the data or notice something is strange or missing, please contact me (Myrthe Tilleman) via email on metillem@ifi.uio.no or in the SINLab Slack channel if you have access to that.